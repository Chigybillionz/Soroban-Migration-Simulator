...
use stellar_xdr::curr::{
    Limited, Limits, ReadXdr, ScEnvMetaEntry, ScMetaEntry, ScSpecEntry, ScSpecTypeDef,
};
use thiserror::Error;
                ...
                if name == "contractspecv0" {
                                ...
                                for case in u.cases.iter() {
                                    match case {
                                        ...
                                        stellar_xdr::curr::ScSpecUdtUnionCaseV0::TupleV0(c) => {
                                            ...
                                            fields.push(FieldAnalysis {
                                                name: c.name.to_utf8_string_lossy().to_string(),
                                                type_ref: TypeRef::Complex(format!(
                                                    "Tuple<{}>",
                                                    type_strs.join(", ")
                                                )),
                                            });
                    ...
                    while (limited.inner.position() as usize) < data.len() {
                        if let Ok(ScMetaEntry::ScMetaV0(m)) = ScMetaEntry::read_xdr(&mut limited) {
                            metadata.insert(
                                m.key.to_utf8_string_lossy().to_string(),
                                m.val.to_utf8_string_lossy().to_string(),
                            );
                        } else {
                            break;
                        }
                    ...
                    while (limited.inner.position() as usize) < data.len() {
                        if let Ok(
                            stellar_xdr::curr::ScEnvMetaEntry::ScEnvMetaKindInterfaceVersion(v),
                        ) = ScEnvMetaEntry::read_xdr(&mut limited)
                        {
                            env_metadata = Some(EnvMetadata {
                                interface_version: v.protocol,
                            });
                        } else {
                            break;
                        }
...
fn map_type_def(type_def: &ScSpecTypeDef) -> TypeRef {
    match type_def {
        ...
        ScSpecTypeDef::Option(o) => TypeRef::Complex(format!(
            "Option<{}>",
            map_type_def(&o.value_type).to_string_rep()
        )),
        ScSpecTypeDef::Result(r) => TypeRef::Complex(format!(
            "Result<{}, {}>",
            map_type_def(&r.ok_type).to_string_rep(),
            map_type_def(&r.error_type).to_string_rep()
        )),
        ScSpecTypeDef::Vec(v) => TypeRef::Complex(format!(
            "Vec<{}>",
            map_type_def(&v.element_type).to_string_rep()
        )),
        ScSpecTypeDef::Map(m) => TypeRef::Complex(format!(
            "Map<{}, {}>",
            map_type_def(&m.key_type).to_string_rep(),
            map_type_def(&m.value_type).to_string_rep()
        )),
...
mod tests {
    ...
    fn test_without_metadata() {
        let wasm = vec![
            0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00, 0x00, 0x0f, 0x0e, b'c', b'o', b'n',
            b't', b'r', b'a', b'c', b't', b's', b'p', b'e', b'c', b'v', b'0',
        ];
    ...
    fn test_record_fields_v2() {
        ...
        let analysis = Analyzer::analyze(&wasm).unwrap();
        let record_v2 = analysis
            .types
            .iter()
            .find(|t| t.name == "RecordV2")
            .unwrap();
